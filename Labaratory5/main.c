...
int main() {
	...
	int Time;
	scanf_s("%d", &Time);
	printf("Write ProgrammSnake, Number 1 = StupidSnake, Number 2 = AdvancedSnake, Number 3 = UserSnake\n");
	int NumberExc;
	scanf_s("%d", &NumberExc);
	if (NumberOfString > 30 && NumberOfString < 10 || NumberOfColumn>30 && NumberOfColumn < 10 || Time>100 && Time < 20 || NumberExc < 1 && NumberExc>3) {
		printf("YOU LIE");
		return 0;
	}
	system("cls");
